...
class Primes
  ...
  def prime_factors(n) # prime_factors(12, primes) = { 2=>2, 3=>1 }
    factors_hash = {}
    list.each do |q|
      factors_hash[q] = 0
...
primes = Primes.new(200)
(1..200).each { |n| p primes.f(n) }

# h = {}
# (4..100).each { |n| h[n] = BooleanMatrix.construct(n, primes).interpret.last }
# p h
