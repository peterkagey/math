...
class GrahamCounter
  ...
  def all_permutations
    ...
    (0...permutation_count).each do |i|
      ...
      puts "#{n}: #{i}/#{permutation_count}"
      @file_string << format_array_to_string(subset)
      counter += 1
    end
  ...
  def subset_forms_perfect_square?(subset)
    subset.map { |k| prime_parity(k) }.reduce(:^) == target_xor
  end

  def target_xor
    @target_xor ||= prime_parity(n) ^ prime_parity(g(n))
  end
...
# p GrahamCounter.binary_to_indices(0b1101)
(37..50).each do |i|
  # GrahamCounter.new(i).count_all_valid_sequences
  next unless Prime.prime?(i)
  counter = GrahamCounter.new(i)
  counter.all_permutations
  file_name = "#{i}".rjust(3, '0')
  message = counter.file_string + ""
  File.write("nonsquare_permutations/#{file_name}.txt", counter.file_string)
end
