...
baseSequences'' n m = map (n:) $ subsequences [n+1..m]
...
sequenceCount b baseSequence = length $ filter (isKthPower b) $ map product $ allPowers b baseSequence
...
aT'' n k = sum $ map (sequenceCount n) $ baseSequences'' k (aT n k)
