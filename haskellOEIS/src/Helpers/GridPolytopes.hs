...
-- polygons with side length k, all nonnegative coordinates, and touching the coordinate planes
normalized :: Polygon -> Integer -> [VertexSet]
normalized polygon k = nub $ map (\[v,u] -> makeShape polygon v u) $ adjacentSides polygon k

-- crossProduct :: Set [Integer] -> (Integer,Integer,Integer)
crossProduct triangle = ([a,b,c], x4^2+y4^2+z4^2, a^2+b^2+c^2) where
  (a,b,c) = (y4*z5-z4*y5, z4*x5-x4*z5, x4*y5-y4*x5)
  ([x4,y4,z4], [x5,y5,z5]) = ([x2 - x1, y2 - y1, z2 - z1],[x3 - x1, y3 - y1, z3 - z1])
  [[x1,y1,z1],[x2,y2,z2],[x3,y3,z3]] = Set.toList triangle
