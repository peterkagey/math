module Helpers.Table (tableByAntidiagonals, triangleByRows, indicesByAntidiagonals, triangularize, antidiagonalize, formatTable) where
...
-- This just makes a table pretty.
formatTable :: Show a => [[a]] -> [[String]]
formatTable as = map (zipWith pad lengths) as where
